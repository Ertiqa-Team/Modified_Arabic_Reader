...
public final class FBView extends ZLTextView {
	...
	public boolean onFingerSingleTap(int x, int y) {
		...
		myReader.runAction(getZoneMap().getActionByCoordinates(
				x, 0, myContext.getWidth(), myContext.getHeight(),
				isDoubleTapSupported() ? TapZoneMap.Tap.singleNotDoubleTap : TapZoneMap.Tap.singleTap
			), x, y);
	...
	private void startManualScrolling(int x, int y) {
		...
		//==================================================
		int h = myContext.getHeight();
		int w = myContext.getWidth();
		int hh = h/4;
		int ww = w/4;
		
		// prevent scrolling animation if the press in the mid of the view
	...
	@Override
	public ZLColor getSearchHighlightingColor() {
		return new ZLColor(255, 81, 81);//light red color... 
	}
