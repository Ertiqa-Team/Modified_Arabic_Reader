...
//	private ZLTextHighlighting myHighlighting;
	private Vector<ZLTextAbstractHighlighting>  myHighlightingVector;
	
	public ZLTextView(ZLApplication application) {
		super(application);
		mySelection = new ZLTextSelection(this);
	//	myHighlightingV = new ZLTextHighlighting();
		myHighlightingVector = new Vector<ZLTextAbstractHighlighting>();
	}
	...
	public void highlight(ZLTextPosition start, ZLTextPosition end) {
		ZLTextHighlighting myHighlighting=new ZLTextHighlighting();
		myHighlighting.setup(start, end);
		myHighlightingVector.add(myHighlighting);
		Application.getViewWidget().reset();
		Application.getViewWidget().repaint();
	}
	
	public void unHighlight(ZLTextPosition start, ZLTextPosition end) {
		for(int i=0;i<myHighlightingVector.size();i++){
			ZLTextAbstractHighlighting hil = myHighlightingVector.elementAt(i);
			if((hil.getStartPosition().compareTo(start)==0) && (hil.getEndPosition().compareTo(end)==0)){
				myHighlightingVector.remove(i);
				break;
			}
		}
		Application.getViewWidget().reset();
	...
	public void clearHighlighting() {
		if(myHighlightingVector == null || myHighlightingVector.size()==0)return;
		for (int i=0;i<myHighlightingVector.size();i++){
			ZLTextAbstractHighlighting myHighlighting = myHighlightingVector.elementAt(i);
	...
	private void drawBackgroung(
		...
		if (!highligting.isEmpty() && from != to) {
			...
			if (selectionStartArea != null
				...
				//======================Arabic selection modify=============================
				if(left!=getLeftMargin() && right==getRightLine()){
					right = getLeftMargin();
				}
				if(left==getLeftMargin() && right != getRightLine()){
					left = getRightLine();
	...
	//============================================================
	private void drawBackgroung(
			Vector<ZLTextAbstractHighlighting> highligtings, ZLColor color,
			ZLTextPage page, ZLTextLineInfo info, int from, int to, int y
		) {
		if(highligtings==null||highligtings.size()==0)return;
		for(int i=0;i<highligtings.size();i++){
			ZLTextAbstractHighlighting highligting = highligtings.elementAt(i);
	...
	private void drawTextLine(ZLTextPage page, ZLTextLineInfo info, int from, int to, int y) {
		...
		drawBackgroung(myHighlightingVector, getHighlightingColor(), page, info, from, to, y);
	...
	@Override
	public boolean canScroll2(PageIndex index) {
		switch (index) {
			default:
				return true;
			case previous:
